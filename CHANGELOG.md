...
**新增原因**: 
- 问题反馈：AI 优先使用通用最佳实践，而非项目特定规范
- 实际案例：chat 项目禁止 Service 层，但 AI 仍然生成了 Service 类
- 根本原因：缺少强制的项目规范检查机制
...
更新位置:
  - 立即执行规则：添加"项目规范优先"规则
  - 关键决策点：添加"开始任何代码任务"检查
  - 目录：添加"场景 0"链接
```

**profiles/chat.md 补充规范**:
```yaml
新增章节: 架构与技术栈强制规范（🔴 最高优先级）

明确5个核心强制规范:
  1. 🔴 架构层次禁止项（禁止 Service 层）
  2. 🔴 参数验证强制规范（强制使用 Joi）
  3. 🔴 测试框架强制规范（强制使用 Mocha）
  4. 🔴 数据库操作强制规范（强制使用 utilsCrud）
  5. 🔴 注释语言强制规范（强制中文注释）
...
**相关文档**: [规范遵守改进总结](docs/规范遵守改进总结.md)

---

### 📊 Added（新增 - 分析报告）

#### 2. copilot-instructions.md 与 v2.md 联动分配深度分析

**分析原因**:
- 用户提问："作为 AI 大模型，两个文件的联动分配是否合理？"
- 需求：从 AI 使用者视角评估职责划分、信息密度、查找效率、维护成本
...
核心问题:
  1. 场景 0 高度重复（95%相同）
  2. 信息密度不均（copilot 冗余 60%，v2.md 场景过简）
  3. 跳转次数过多（10次 → 建议优化到3次）
  4. 维护成本高（12处需要同步 → 建议优化到7处）

优化建议:
  - 🔴 优先级1: 极简化 copilot（1000行→300行）
  - 🔴 优先级1: 减少场景 0 重复
  - 🟠 优先级2: 统一场景描述格式
  - 🟠 优先级2: 引入场景决策矩阵
  - 🟡 优先级3: 增加常见错误章节
...
**已创建文档**:
- [copilot-v2-联动分配深度分析.md](docs/copilot-v2-联动分配深度分析.md) - 详细分析（5000+字）
- [copilot-v3-优化实施方案.md](docs/copilot-v3-优化实施方案.md) - v3.0 优化版本
- [copilot-v2-联动验证清单.md](docs/copilot-v2-联动验证清单.md) - 一致性验证
...
#### 3. copilot-instructions.md 与 v2.md 不一致问题修复
...
问题3: 禁止行为数量不一致
  修复前: 4条禁止行为
  修复后: 5条禁止行为（补充"使用项目明确禁止的技术栈"）
  影响: 禁止项更完整
```

**修复文件**:
- `.github/copilot-instructions.md`（3处修改）
...
**验证文档**: [copilot-v2-严格验证报告.md](docs/copilot-v2-严格验证报告.md)
...
**详细内容**:
- ✅ .gitignore 配置示例（运行时文件 vs 应提交文件）
- ✅ 注意事项（敏感信息检查、文件大小控制、命名规范）
- ✅ 提交最佳实践（与代码一起提交、PR 引用分析文档）
- ✅ 业界参考（ADR、Rust RFC、Python PEP）

**注意事项**:
- ⚠️ 敏感信息检查：不应包含真实密码、token、用户数据
- ⚠️ 文件大小控制：单个文件建议 < 1MB，避免大量截图
- ⚠️ 命名规范：使用 kebab-case，避免特殊字符和中文路径

**影响范围**:
- 现有项目：monSQLize、guidelines 项目已正确提交这些目录 ✅
- 新项目：参考 .gitignore 配置模板

#### 2. 第21章补充：测试目录结构详细规范（按测试类型分类）
