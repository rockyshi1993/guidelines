...
**核心原则**：
- 🔴 **代码即文档**（Code as Documentation）- 通过注解/装饰器生成文档，避免手写维护
- 🔴 **类型安全**（Type Safety）- 自动校验请求/响应类型，减少运行时错误
- 🟠 **版本管理**（API Versioning）- 明确 API 版本，支持平滑迁移
- 🟡 **测试便利**（Testing）- 自动生成测试工具（Swagger UI / Postman）

#### 强制要求（API 项目）

```yaml
必须提供:
  1. 🔴 OpenAPI 规范文件（swagger.json 或 openapi.yaml）
     位置: <项目根>/docs/api/swagger.json 或 openapi.yaml
...
const swaggerOptions = {
  ...
  apis: ['./routes/*.js', './controllers/*.js'] // 扫描路径
};
...
async def get_user(user_id: str):
    ...
    """
    # 实现逻辑
    return User(id=user_id, name="张三", email="zhang@example.com")
```
