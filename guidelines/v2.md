...
19. [临时文档清理规范](#19-临时文档清理规范) - 文档精简
    ...
    - 19.2 [改进方案设计规范](#192-改进方案设计规范强制) - 方案设计模板与流程
    - 19.3 [项目标准目录结构规范](#193-项目标准目录结构规范) - 标准目录结构
...
1. 禁止删除现有的import语句
   ...
   示例错误: 删除 "import selectorGroup from './selector'"
   后果: selectorGroup路由立即404，用户无法访问该功能
   
2. 禁止删除现有的路由调用
   原因: 删除路由调用等同于下线功能，影响所有用户
   示例错误: 删除 "selectorGroup(app, _groupRouter)"
   后果: 该路由组下的所有接口立即不可用
...
本规范强制适用于所有**主动性改进/优化任务**（非 Bug 响应）：
...
```yaml
强制流程:
  1. 🔴 创建方案文档: <项目>/analysis-reports/YYYY-MM-DD-主题.md
  2. 🔴 完整填写方案内容（见下方模板）
  3. 🔴 等待用户明确确认
  4. ✅ 确认后开始实施
...
**文件位置**: `<项目>/analysis-reports/YYYY-MM-DD-<类型>-<简短描述>.md`

**命名示例**:
- `2025-11-11-refactor-user-service.md`
- `2025-11-11-performance-cache-optimization.md`
- `2025-11-11-feature-add-pagination.md`
...
| 局限性 | 具体表现 | 影响程度 | 是否可通过配置/参数解决？ |
|--------|---------|---------|----------------------|
| 局限1 | 描述 | 高/中/低 | 是/否，原因 |
| 局限2 | 描述 | 高/中/低 | 是/否，原因 |

### 2.3 为什么不能最小改动？（如果不使用最小改动方案）
**仅在建议非最小改动方案时填写**:
...
**🔴 强制要求**: 必须提供至少2个方案，其中必须包含"最小改动方案"（除非有充分理由）

#### 方案 A: 最小改动方案（推荐）

**核心思路**: 在现有架构上修改，最小化影响范围
...
| 文件路径 | 操作类型 | 改动说明 | 风险等级 |
|---------|---------|---------|---------|
| src/xxx.js | 修改 | 描述 | 高/中/低 |
| src/yyy.js | 新增 | 描述 | 高/中/低 |
...
```yaml
当AI识别到改进任务时:
  1. 立即创建方案文档（analysis-reports/YYYY-MM-DD-主题.md）
  2. 按模板填写完整内容
  3. 明确输出: "✅ 方案已创建: <路径>"
  4. 等待用户确认: "请确认方案后，我再开始实施"
...
```yaml
用户选项:
  ✅ 同意: "方案可行，开始实施" → AI 进入实施阶段
  🔄 修改: "方案A改为XXX" → AI 更新方案，重新确认
  ❌ 拒绝: "不采纳" → AI 不执行任何代码改动
```
...
AI 执行:
  1. ✅ 创建: analysis-reports/2025-11-11-performance-user-query.md
  2. ✅ 方案A（最小改动）: 添加索引
  3. ✅ 方案B（局部重构）: 引入缓存层
  4. ✅ 方案C（大规模）: 重构整个查询模块（不推荐）
  5. ✅ 推荐: 方案A（影响小，风险低）
  6. ⏸️ 等待用户确认
  
❌ 禁止: 直接说"我来优化查询代码"然后立即修改
```
...
AI 执行:
  1. ✅ 创建: analysis-reports/2025-11-11-refactor-user-module.md
  2. ✅ 用户明确要求重构 → 可以建议大规模重构
  3. ✅ 方案A（最小改动）: 优化现有代码
  4. ✅ 方案B（局部重构）: 拆分大文件
  5. ✅ 方案C（大规模）: 完全重写（用户要求）
  6. ✅ 推荐: 根据具体情况
...
AI 执行:
  1. ✅ 创建: analysis-reports/2025-11-11-feature-add-pagination.md
  2. ✅ 检查: 现有代码是否支持扩展？
  3. ✅ 方案A（最小改动）: 在现有查询函数添加 limit/skip 参数
  4. ✅ 方案B（局部重构）: 创建通用分页工具
  5. ✅ 推荐: 方案A（不影响现有功能）
  6. ⏸️ 等待用户确认
...
```yaml
❌ 错误做法:
  AI: "我建议重构用户模块，我现在开始修改代码..."
  [直接修改代码]

✅ 正确做法:
  AI: "我建议重构用户模块，已创建方案文档: 
      analysis-reports/2025-11-11-refactor-user-module.md
...
| 规范 | 关系 | 说明 |
|------|------|------|
| **第19.1章** | 目录规范 | 本章定义内容规范，第19.1章定义目录结构 |
| **场景F** | 执行规范 | copilot-instructions.md 场景F 强制执行本章规则 |
| **第3章** | 提交规范 | 方案确认后，提交信息应引用方案文档 |
| **第6章** | 文档联动 | 实施后需同步更新 CHANGELOG/README |
| **第11章** | 兼容性 | 方案设计需考虑向后兼容性 |
