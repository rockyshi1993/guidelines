...
理由:
  1. 知识沉淀: 团队共享的宝贵知识，防止经验流失
  2. 历史追溯: 记录项目演进过程，了解为什么这样做
  3. 技术决策: 记录方案评估和选择理由
  4. 经验传承: 新成员快速了解历史问题和决策背景
  5. 代码关联: 与代码变更形成完整历史，PR 可引用分析文档

类比:
  - 类似于 docs/ (项目文档，应该提交)
  - 类似于 ADR (Architecture Decision Records)
  - 类似于 CHANGELOG.md (变更历史)
  - 不同于 logs/ (运行日志，不应提交)
  - 不同于 coverage/ (覆盖率报告，不应提交)
```
...
1. **敏感信息检查**:
   ...
   使用:
     ✅ 占位符: "mongodb://username:password@host/db"
     ✅ 脱敏数据: "用户ID: user_12345"
     ✅ 示例数据: "测试环境配置"
   ```
...
4. **提交最佳实践**:
   ...
   bug-analysis/:
     - 与 Bug 修复代码一起提交
     - PR 中引用对应的分析文档
     - Commit message 关联: "fix: 修复连接泄漏问题 (参考 bug-analysis/2025-11-04-connection-leak.md)"
   
   analysis-reports/:
     - 与改进代码一起提交（或稍早提交）
     - PR 中说明分析报告位置
     - Commit message 关联: "feat: 实施 P0 改进 (参考 analysis-reports/P0-improvements-report.md)"
   ```
...
- **Python PEP**: https://github.com/python/peps
...
**要点**：
- 🔴 强制：每个公开 API 都应有对应文档
- 🟠 必须：文档包含参数说明、使用示例、返回值
- 🟡 推荐：添加注意事项和性能建议
- 详细文档结构参见 [第6章 代码修改与文档联动](#6-代码修改与文档联动)
