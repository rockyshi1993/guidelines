...
> **重要**: 以下所有目录路径都是相对于**项目根目录**

### 项目根目录判断规则（按优先级）:

1. **Profile 指定的项目路径**（最高优先级）
   - 如果 `profiles/{project}.md` 中明确指定了项目路径，使用该路径

2. **当前工作目录中最近的 package.json 所在目录**
   - 从当前工作目录向上查找，直到找到 package.json
   - 适用于 Node.js/JavaScript/TypeScript 项目

3. **当前工作目录中最近的 Git 仓库根目录**
   - 从当前工作目录向上查找，直到找到 .git 目录
   - 适用于所有使用 Git 的项目
...
如果工作区包含多个子项目（例如 `packages/app1/`, `packages/app2/`），AI 应该工作在**当前操作的子项目根目录**，而非工作区根目录。

**示例**:
- **单项目**: 项目在 `D:/projects/my-app/` → 项目根目录 = `D:/projects/my-app/`
- **Monorepo**: 工作区在 `D:/workspace/`，当前子项目在 `D:/workspace/packages/app1/` → 项目根目录 = `D:/workspace/packages/app1/`

**判断依据**: AI 应根据用户当前打开的文件或明确指定的模块来确定正在操作哪个子项目。
