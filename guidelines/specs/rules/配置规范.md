# v3 配置规范

> **文件**: specs/rules/配置规范.md  
> **版本**: v3.0  
> **日期**: 2025-11-20  
> **说明**: 配置文件的管理规范

---

## 📑 目录导航

> 🔴 = 必须遵守 | 🟡 = 建议遵守 | 🟢 = 参考

- [配置文件类型](#-配置文件类型) 🔴 - 环境配置/应用配置
  - [环境配置](#环境配置) 🔴 - .env文件管理
  - [应用配置](#应用配置) 🟡 - 应用特定配置
- [环境变量命名规范](#-环境变量命名规范) 🔴 - UPPER_SNAKE_CASE
- [.env 文件管理](#-env-文件管理) 🔴 - 必须有.env.example
  - [.env 文件内容](#env-文件内容) 🔴 - 真实配置(禁止提交)
  - [.env.example 模板](#envexample-模板) 🔴 - 示例模板(必须提交)
- [配置加载顺序](#-配置加载顺序) 🔴 - 默认→环境→.env→命令行
- [敏感信息管理](#-敏感信息管理) 🔴 - 禁止硬编码
  - [禁止硬编码](#禁止硬编码) 🔴 - 任何敏感信息都不能硬编码
  - [使用环境变量](#使用环境变量) 🔴 - process.env.XXX
  - [生产环境密钥管理](#生产环境密钥管理) 🔴 - 使用密钥管理服务
- [配置验证](#-配置验证) 🔴 - 启动时验证所有必需配置
- [配置文档](#-配置文档) 🟡 - 记录配置说明
- [Profile 优先](#-profile-优先) 🟡 - 项目可自定义配置规范

---

## 📋 配置文件类型

### 环境配置

```yaml
.env 文件（禁止提交）:
  - .env: 本地开发环境
  - .env.local: 本地覆盖配置
  - .env.development: 开发环境（可提交模板）
  - .env.test: 测试环境
  - .env.staging: 预发布环境
  - .env.production: 生产环境

.env.example（必须提交）:
  - 提供所有环境变量的模板
  - 不包含真实值
  - 包含注释说明
```

### 应用配置

```yaml
configs/ 目录:
  - config.json: 默认配置
  - config.dev.json: 开发环境配置
  - config.test.json: 测试环境配置
  - config.staging.json: 预发布环境配置
  - config.prod.json: 生产环境配置

package.json:
  - 项目元信息
  - 依赖管理
  - 脚本命令

tsconfig.json / jsconfig.json:
  - TypeScript/JavaScript 配置

.eslintrc.json:
  - 代码检查规则

.prettierrc.json:
  - 代码格式化规则
```

---

## 🔒 环境变量规范

### .env 文件格式

```env
# 数据库配置
DB_HOST=localhost
DB_PORT=5432
DB_NAME=myapp
DB_USER=admin
DB_PASSWORD=your_secure_password_here

# Redis配置
REDIS_HOST=localhost
REDIS_PORT=6379
REDIS_PASSWORD=

# API密钥（禁止提交真实密钥）
STRIPE_API_KEY=sk_test_xxxxxxxxxxxxxxxxxxxx
GOOGLE_API_KEY=AIzaxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx

# JWT配置
JWT_SECRET=your_jwt_secret_here
JWT_EXPIRES_IN=24h
JWT_REFRESH_EXPIRES_IN=7d

# 应用配置
NODE_ENV=development
PORT=3000
API_URL=http://localhost:3000
FRONTEND_URL=http://localhost:8080

# 日志配置
LOG_LEVEL=debug
LOG_FILE=logs/app.log

# 邮件配置
SMTP_HOST=smtp.example.com
SMTP_PORT=587
SMTP_USER=no-reply@example.com
SMTP_PASSWORD=your_smtp_password

# 第三方服务
AWS_ACCESS_KEY_ID=AKIAIOSFODNN7EXAMPLE
AWS_SECRET_ACCESS_KEY=wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY
AWS_REGION=us-east-1
```

### .env.example 模板

```env
# =================================
# 环境变量配置模板
# 复制此文件为 .env 并填入实际值
# =================================

# 数据库配置
DB_HOST=localhost
DB_PORT=5432
DB_NAME=
DB_USER=
DB_PASSWORD=

# Redis配置
REDIS_HOST=localhost
REDIS_PORT=6379
REDIS_PASSWORD=

# API密钥
STRIPE_API_KEY=
GOOGLE_API_KEY=

# JWT配置
JWT_SECRET=
JWT_EXPIRES_IN=24h
JWT_REFRESH_EXPIRES_IN=7d

# 应用配置
NODE_ENV=development
PORT=3000
API_URL=
FRONTEND_URL=

# 日志配置
LOG_LEVEL=info
LOG_FILE=logs/app.log

# 邮件配置
SMTP_HOST=
SMTP_PORT=587
SMTP_USER=
SMTP_PASSWORD=

# 第三方服务
AWS_ACCESS_KEY_ID=
AWS_SECRET_ACCESS_KEY=
AWS_REGION=us-east-1
```

### 命名规范

```yaml
环境变量命名:
  格式: UPPER_SNAKE_CASE
  示例: DB_HOST, API_KEY, JWT_SECRET

分组:
  使用前缀分组相关配置:
    DB_*: 数据库相关
    REDIS_*: Redis相关
    JWT_*: JWT相关
    SMTP_*: 邮件相关
    AWS_*: AWS相关

布尔值:
  使用: true/false
  示例: ENABLE_CACHE=true

数字:
  不使用引号
  示例: PORT=3000

字符串:
  可选引号
  示例: DB_NAME=myapp 或 DB_NAME="myapp"
```

---

## 📄 应用配置规范

### config.json 结构

```json
{
  "app": {
    "name": "MyApp",
    "version": "1.0.0",
    "env": "development"
  },
  "server": {
    "port": 3000,
    "host": "localhost",
    "cors": {
      "enabled": true,
      "origins": ["http://localhost:8080"]
    }
  },
  "database": {
    "type": "postgres",
    "host": "localhost",
    "port": 5432,
    "name": "myapp",
    "pool": {
      "min": 2,
      "max": 10
    },
    "ssl": false
  },
  "redis": {
    "host": "localhost",
    "port": 6379,
    "db": 0,
    "keyPrefix": "myapp:"
  },
  "jwt": {
    "expiresIn": "24h",
    "refreshExpiresIn": "7d",
    "issuer": "myapp"
  },
  "logging": {
    "level": "info",
    "format": "json",
    "output": ["console", "file"],
    "file": "logs/app.log"
  },
  "cache": {
    "enabled": true,
    "ttl": 3600,
    "maxSize": 1000
  },
  "rateLimit": {
    "enabled": true,
    "windowMs": 900000,
    "max": 100
  }
}
```

### 环境特定配置

```json
// config.dev.json
{
  "app": {
    "env": "development"
  },
  "server": {
    "port": 3000
  },
  "database": {
    "host": "localhost",
    "name": "myapp_dev",
    "ssl": false
  },
  "logging": {
    "level": "debug"
  }
}

// config.prod.json
{
  "app": {
    "env": "production"
  },
  "server": {
    "port": 8080,
    "cors": {
      "origins": ["https://example.com"]
    }
  },
  "database": {
    "ssl": true,
    "pool": {
      "min": 10,
      "max": 50
    }
  },
  "logging": {
    "level": "warn",
    "format": "json"
  }
}
```

### 配置加载顺序

```yaml
加载顺序（后者覆盖前者）:
  1. config.json（默认配置）
  2. config.{env}.json（环境特定配置）
  3. 环境变量（.env文件）
  4. 命令行参数

示例:
  const config = {
    ...defaultConfig,
    ...envConfig,
    ...processEnv,
    ...cliArgs
  };
```

---

## 🔐 敏感信息管理

### 禁止硬编码

```yaml
❌ 禁止在配置文件中硬编码:
  - 密码
  - API密钥
  - 数据库连接字符串（含密码）
  - 私钥
  - 加密密钥
  - 生产环境URL

✅ 必须使用环境变量:
  const password = process.env.DB_PASSWORD;
  const apiKey = process.env.STRIPE_API_KEY;
```

### .gitignore 配置

```gitignore
# 环境变量文件
.env
.env.local
.env.*.local

# 不要忽略示例文件
!.env.example

# 日志文件
logs/
*.log

# 临时文件
tmp/
temp/
.cache/

# 依赖
node_modules/

# 构建产物
dist/
build/

# IDE配置
.vscode/
.idea/
*.swp
*.swo

# OS文件
.DS_Store
Thumbs.db
```

### 密钥管理最佳实践

```yaml
开发环境:
  - 使用 .env 文件
  - 不提交到Git

测试环境:
  - 使用CI/CD配置的环境变量
  - 或使用密钥管理服务

生产环境:
  - 使用密钥管理服务:
    - AWS Secrets Manager
    - Azure Key Vault
    - HashiCorp Vault
  - 使用环境变量（容器/云平台）
  - 定期轮换密钥
```

---

## ⚙️ 配置验证

### 启动时验证

```javascript
// config/validator.js

const requiredEnvVars = [
  'DB_HOST',
  'DB_PORT',
  'DB_NAME',
  'DB_USER',
  'DB_PASSWORD',
  'JWT_SECRET',
  'REDIS_HOST'
];

function validateConfig() {
  const missing = [];
  
  for (const varName of requiredEnvVars) {
    if (!process.env[varName]) {
      missing.push(varName);
    }
  }
  
  if (missing.length > 0) {
    throw new Error(
      `Missing required environment variables: ${missing.join(', ')}\n` +
      `Please check your .env file.`
    );
  }
  
  // 验证格式
  const port = parseInt(process.env.DB_PORT);
  if (isNaN(port) || port < 1 || port > 65535) {
    throw new Error('DB_PORT must be a valid port number');
  }
  
  // 验证枚举值
  const validEnvs = ['development', 'test', 'staging', 'production'];
  if (!validEnvs.includes(process.env.NODE_ENV)) {
    throw new Error(`NODE_ENV must be one of: ${validEnvs.join(', ')}`);
  }
}

module.exports = { validateConfig };
```

### 配置Schema

```javascript
// config/schema.js

const Joi = require('joi');

const configSchema = Joi.object({
  app: Joi.object({
    name: Joi.string().required(),
    version: Joi.string().pattern(/^\d+\.\d+\.\d+$/).required(),
    env: Joi.string().valid('development', 'test', 'staging', 'production').required()
  }),
  server: Joi.object({
    port: Joi.number().port().required(),
    host: Joi.string().hostname().required(),
    cors: Joi.object({
      enabled: Joi.boolean().required(),
      origins: Joi.array().items(Joi.string().uri())
    })
  }),
  database: Joi.object({
    type: Joi.string().valid('postgres', 'mysql', 'mongodb').required(),
    host: Joi.string().hostname().required(),
    port: Joi.number().port().required(),
    name: Joi.string().required(),
    pool: Joi.object({
      min: Joi.number().min(1).required(),
      max: Joi.number().min(1).required()
    })
  })
});

function validateConfigSchema(config) {
  const { error, value } = configSchema.validate(config);
  if (error) {
    throw new Error(`Config validation error: ${error.message}`);
  }
  return value;
}

module.exports = { validateConfigSchema };
```

---

## 📝 配置文档

### 配置说明文档

```markdown
# 配置说明

## 环境变量

### 数据库配置

| 变量名 | 类型 | 必填 | 默认值 | 说明 |
|-------|------|------|--------|------|
| DB_HOST | string | 是 | - | 数据库主机地址 |
| DB_PORT | number | 是 | - | 数据库端口 |
| DB_NAME | string | 是 | - | 数据库名称 |
| DB_USER | string | 是 | - | 数据库用户名 |
| DB_PASSWORD | string | 是 | - | 数据库密码 |

### Redis配置

| 变量名 | 类型 | 必填 | 默认值 | 说明 |
|-------|------|------|--------|------|
| REDIS_HOST | string | 是 | localhost | Redis主机地址 |
| REDIS_PORT | number | 否 | 6379 | Redis端口 |
| REDIS_PASSWORD | string | 否 | - | Redis密码 |

### JWT配置

| 变量名 | 类型 | 必填 | 默认值 | 说明 |
|-------|------|------|--------|------|
| JWT_SECRET | string | 是 | - | JWT签名密钥 |
| JWT_EXPIRES_IN | string | 否 | 24h | Token过期时间 |
| JWT_REFRESH_EXPIRES_IN | string | 否 | 7d | RefreshToken过期时间 |

## 应用配置

### server配置

\`\`\`json
{
  "server": {
    "port": 3000,         // 服务器端口
    "host": "localhost",  // 服务器主机
    "cors": {
      "enabled": true,    // 是否启用CORS
      "origins": []       // 允许的源
    }
  }
}
\`\`\`

### logging配置

\`\`\`json
{
  "logging": {
    "level": "info",      // 日志级别: debug/info/warn/error
    "format": "json",     // 日志格式: json/text
    "output": ["console"] // 输出目标: console/file
  }
}
\`\`\`
```

---

## 🔄 配置更新流程

### Intent-09: 配置修改

```yaml
执行流程:
  STEP 5-7: 分析和确认
    1. 分析配置需求
    2. 生成配置方案
    3. 用户确认方案
  
  STEP 9: 修改配置文件
    - configs/config.{env}.json
    - .env.example（如果新增环境变量）
  
  STEP 10: 用户确认配置
  
  STEP 15: 更新文档
    - 更新配置说明文档
    - 更新README.md（如果需要）
  
  STEP 17: 验证配置
    - 验证JSON格式
    - 验证配置schema
    - 验证环境变量
```

### 配置变更检查

```yaml
变更前检查:
  - ✅ 配置项是否已存在
  - ✅ 默认值是否合理
  - ✅ 是否影响现有功能
  - ✅ 是否需要迁移

变更后验证:
  - ✅ JSON格式正确
  - ✅ Schema验证通过
  - ✅ 应用能正常启动
  - ✅ 功能测试通过
```

---

## 📊 配置最佳实践

### 配置分层

```yaml
三层配置:
  1. 默认配置（config.json）
     - 通用配置
     - 开发友好的默认值
  
  2. 环境配置（config.{env}.json）
     - 环境特定的值
     - 覆盖默认配置
  
  3. 运行时配置（环境变量）
     - 敏感信息
     - 部署时的动态值
     - 覆盖所有配置
```

### 配置类型

```yaml
三种类型:
  1. 静态配置（提交到Git）
     - 应用名称
     - 版本号
     - 默认端口
     - 功能开关默认值
  
  2. 环境特定配置（提交到Git）
     - 日志级别
     - 连接池大小
     - 超时时间
  
  3. 敏感配置（不提交到Git）
     - 密码
     - API密钥
     - 数据库连接字符串
```

### 配置原则

```yaml
原则:
  1. 约定优于配置
     - 提供合理的默认值
     - 减少必需配置项
  
  2. 环境分离
     - 开发/测试/生产环境隔离
     - 使用不同的配置文件
  
  3. 安全第一
     - 敏感信息不提交
     - 使用环境变量
     - 生产环境使用密钥管理服务
  
  4. 验证优先
     - 启动时验证配置
     - 失败快速（Fail Fast）
     - 清晰的错误信息
  
  5. 文档完整
     - 所有配置项都有说明
     - .env.example 完整
     - 更新及时
```

---

## 🛠️ 配置工具

### 推荐工具

```yaml
环境变量管理:
  - dotenv: 加载.env文件
  - dotenv-expand: 支持变量展开
  - dotenv-safe: 验证必需变量

配置验证:
  - joi: Schema验证
  - ajv: JSON Schema验证
  - yup: 对象验证

配置管理:
  - config: 多环境配置管理
  - nconf: 分层配置
  - convict: 配置验证和文档

密钥管理:
  - aws-sdk (Secrets Manager)
  - @azure/keyvault-secrets
  - vault: HashiCorp Vault客户端
```

### 使用示例

```javascript
// 加载环境变量
require('dotenv').config();

// 加载配置
const config = require('config');

// 验证配置
const { validateConfig } = require('./config/validator');
validateConfig();

// 使用配置
const dbConfig = config.get('database');
const connection = createConnection(dbConfig);
```

---

**文件创建**: 2025-11-20  
**最后更新**: 2025-11-20  
**状态**: ✅ 完整

